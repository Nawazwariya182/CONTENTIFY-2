...
function CreateNewContent(props: PROPS) {
  ...
  const GenerateAIContent = async (FormData: any) => {
    ...
    try {
      ...
      setaioutput(result);
      await SaveOutputToDB(JSON.stringify(FormData), selectedtemplate?.slug, result);
    } catch (error) {
  ...
  return (
    <div className='p-10'>
      <Link href={'/dashboard'}>
        <Button className='bg-prim text-back hover:bg-white hover:text-text hover:border-2 hover:border-acc w-20 transition-all' style={{ cursor: 'url(/poin.png), auto' }}>
          <ArrowLeft /> Back
        </Button>
      </Link>
